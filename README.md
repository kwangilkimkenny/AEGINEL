...
> ChatGPT, Claude, Gemini에서 실시간으로 프롬프트 위험을 탐지하고, 개인정보(PII)를 자동 가명화하여 보호하는 Chrome 확장 프로그램
...
│  ├─ Input Watching        │    │  ├─ RiskMeter (SVG)     │
│  │  (debounce 300ms)      │    │  ├─ RecentScans         │
│  ├─ Enter Key Intercept   │    │  └─ SettingsPanel       │
│  ├─ Button Click Intercept│    │     (9 layers, PII,     │
│  ├─ Warning Banner        │    │      proxy, threshold)  │
│  │  (Shadow DOM)          │    │                         │
│  ├─ Block Modal           │    └─────────────────────────┘
│  ├─ Proxy Confirm Modal   │
│  ├─ Protected Banner      │
...
5. ChatGPT, Claude, 또는 Gemini 접속하여 테스트
...
각 LLM 사이트의 DOM 구조에 맞게 커스텀 어댑터를 사용합니다.
...
│   │       ├── chatgpt.ts              # ChatGPT 어댑터
│   │       ├── claude.ts               # Claude 어댑터
